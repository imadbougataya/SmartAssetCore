...
public class SystemEventResource {
     ...
     */
    @GetMapping("")
    public ResponseEntity<List<SystemEventDTO>> getAllSystemEvents(SystemEventCriteria criteria) {
        LOG.debug("REST request to get SystemEvents by criteria: {}", criteria);

        List<SystemEventDTO> entityList = systemEventQueryService.findByCriteria(criteria);
