...
import com.ailab.smartasset.service.GatewayQueryService;
...
/**
 * REST controller for managing {@link com.ailab.smartasset.domain.Gateway}.
 */
@RestController
...
public class GatewayResource {
    ...
    public ResponseEntity<GatewayDTO> createGateway(@Valid @RequestBody GatewayDTO gatewayDTO) throws URISyntaxException {
        ...
        gatewayDTO = gatewayService.save(gatewayDTO);
        return ResponseEntity.created(new URI("/api/gateways/" + gatewayDTO.getId()))
            .headers(HeaderUtil.createEntityCreationAlert(applicationName, true, ENTITY_NAME, gatewayDTO.getId().toString()))
            .body(gatewayDTO);
    }
    ...
    public ResponseEntity<GatewayDTO> updateGateway(
        ...
        gatewayDTO = gatewayService.update(gatewayDTO);
     ...
     */
    @GetMapping("/{id}")
    public ResponseEntity<GatewayDTO> getGateway(@PathVariable("id") Long id) {
     ...
     */
    @DeleteMapping("/{id}")
    public ResponseEntity<Void> deleteGateway(@PathVariable("id") Long id) {
